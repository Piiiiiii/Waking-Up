...
3. 优点
- 长连接：不用频繁握手挥手，节约CPU时间和内存，降低网络阻塞
- 短连接：不会占用太多服务端资源，web后端一般为短连接
...
2. 服务器从中选出一组加密算法与HASH算法，并将自己的身份信息以**证书**的形式发回给浏览器。收到证书之后，客户端验证服务器的合法性，包括：证书是否过期，CA 是否可靠，证书有效性检验，服务器证书上的域名是否和服务器的实际域名相匹配；

   - 证书包含了：
   1. Issuer (证书的发布机构)
   指出是什么机构发布的这个证书，也就是指明这个证书是哪个公司创建的(只是创建证书，不是指证书的使用者)。对于上面的这个证书来说，就是指"SecureTrust CA"这个机构。
   2. Valid from , Valid to (证书的有效期)
   3. Public key (A公钥，A私钥在服务器)
   4. Subject (主题)
   这个证书是发布给谁的，或者说证书的所有者，一般是某个人或者某个公司名称、机构的名称、公司网站的网址等。 对于这里的证书来说，是Trustwave Company。
   ...
   这个是用来保证证书的完整性的，也就是说确保证书没有被修改过。 其原理就是在发布证书时，发布者根据指纹算法(一个hash算法)计算整个证书的hash值(指纹/摘要)并和证书放在一起，使用者在打开证书时，自己也根据指纹算法计算一下证书的hash值(指纹)，如果和刚开始的值对得上，就说明证书没有被修改过，因为证书的内容被修改后，根据证书的内容计算的出的hash值(指纹)是会变化的。 注意，这个指纹会使用"CA"证书机构的私钥用签名算法(Signature algorithm)加密后和证书放在一起。
   
   - 证书的有效性检验：
   1. 首先在本地电脑寻找是否有这个服务器证书上的ca机构的证书链。如果有继续下一步，如果没有弹出警告，如果没有且不信任则终止认证。
   2. 使用ca机构根证书的公钥对服务器证书的指纹（明文摘要得到的短文）和指纹算法（摘要算法）进行解密。
   3. 得到指纹算法之后，拿着这个指纹算法对服务器证书的摘要进行计算得到指纹。
   4. 将计算出的指纹和从服务器证书中解密出的指纹对比看是否一样；不同则终止认证

3. 如果证书受信任，或者用户接收了不受信任的证书，浏览器会生成一个**随机密钥B**（用于对称算法），并用服务器提供的A公钥加密（采用非对称算法对密钥加密）（Client Key Exchange）；使用Hash算法对握手消息进行**摘要**计算（Change Cipher Spec），并对摘要使用之前产生的密钥B加密（对称算法）。将加密后的随机密钥B和摘要一起发送给服务器；
4. 服务器使用自己的A私钥解密，得到对称加密的密钥B，用这个密钥B解密出Hash摘要值，并验证握手消息是否一致（Change Cipher Spec）；如果一致，服务器使用对称加密的密钥B加密握手消息发给浏览器；
5. 浏览器解密并验证摘要，若一致，则握手结束。之后的数据传送都使用对称加密的密钥B进行加密
